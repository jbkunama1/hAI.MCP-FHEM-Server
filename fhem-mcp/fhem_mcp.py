...
FHEM_AUTH = os.getenv("FHEM_AUTH")  # Optional: "user:password" for HTTP Basic Auth
...
async def _fhem_async(cmd: str, instance_id: int = None, instance_name: str = None) -> str:
    ...
    # Priority: instance api_key > global FHEM_AUTH > none
    if instance and instance[3]:  # api_key
        headers["Authorization"] = f"Bearer {instance[3]}"
    elif FHEM_AUTH and ":" in FHEM_AUTH:
        # HTTP Basic Auth
        auth_bytes = FHEM_AUTH.encode("utf-8")
