...
- Optional: Wenn FHEMWEB mit HTTP Basic Auth geschützt ist, hat der Betreiber `FHEM_AUTH` gesetzt.

Behandle die MCP-URL, den API-Key und `FHEM_AUTH` (falls vorhanden) als vertrauliche Informationen und gib sie nicht aus.
...
### Hinweis zu FHEM Basic Auth (FHEM_AUTH)

Wenn der Betreiber `FHEM_AUTH` gesetzt hat, sendet der MCP-Server bei Requests an FHEM einen `Authorization: Basic`-Header. Dies ist transparent für dich als Agent – du musst nichts zusätzlich tun. Die Basic-Auth-Konfiguration betrifft nur die Kommunikation zwischen MCP-Server und FHEM, nicht deine Verbindung zum MCP-Server.
...
- Die FHEM-Basic-Auth-Konfiguration (`FHEM_AUTH`) ist transparent für dich – sie betrifft nur die Kommunikation zwischen MCP-Server und FHEM.
